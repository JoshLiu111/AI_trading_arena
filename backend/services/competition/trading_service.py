...
class TradingService:
    def execute_trade(
        ...
        if not account:
            logger.error(f"execute_trade: Account {account_id} not found")
            return None
        
        # Get current price (pass db for test mode)
        price = stock_price_service.get_current_price(ticker, db=db)
        if not price:
            logger.error(f"execute_trade: Failed to get price for {ticker}")
            return None
    ...
    def calculate_total_value(
        ...
        db: Session, 
        account_id: int, 
        pre_fetched_prices: Optional[Dict[str, Optional[float]]] = None
    ) -> float:
        """
        ...
        Args:
            ...
            pre_fetched_prices: Optional dict of ticker -> price to avoid N+1 queries
        """
        account = get_account(db, account_id)
        if not account:
            return 0.0
        ...
        # Fetch prices (bulk if not pre-fetched)
        if pre_fetched_prices is None:
            prices = stock_price_service.get_current_prices_bulk(tickers, db=db)
        else:
            prices = pre_fetched_prices
